...
**Wall time:** `firedrake-ts` uses PETSc’s **adaptive** time stepper by default. At **low** \(Ra\) the step often **grows** (few hundred steps to \(t_{\max}\)); at **high** \(Ra\) the DAE is **stiff**, the step **shrinks**, and part **(b)** can take **many more** TS iterations than **(a)** even for the same \(t_{\max}\).

**Bigger `DT` / fewer steps:** with **adaptive** TS, `DT` is mainly the **starting** nominal step — the controller shrinks/grows \(\Delta t\). To reach \(t_{\max}=10^5\) in **few** steps explicitly, disable adaptivity and set a large fixed step (**may be unstable or wrong** at high \(Ra\) — try on part (a) first):

```bash
TS_FIXED_STEP=1 DT=500 TMAX_4B=100000 hw4/scripts/run_q4_assignment.sh   # ~200 steps/run if it converges
```

Otherwise keep adaptivity and **loosen tolerances** (often expands \(\Delta t\)) rather than forcing huge fixed steps.
...
# Hard cap on PETSc time-step count (run may stop before t_max; check last time in CSV):
TS_MAX_STEPS=50000 hw4/scripts/run_q4_assignment.sh
```

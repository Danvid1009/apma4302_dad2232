...
\medskip
\noindent\textbf{Build and run.} Set \texttt{PETSC\_DIR} to the PETSc root and \texttt{PETSC\_ARCH=apma4302-pkgs-opt} (make and make check passed for this arch). Then:
\begin{verbatim}
make bvp
...
Iteration counts from runs with default \texttt{options\_file} ($m=201$, $\gamma=0$, $k=5$, $c=3$; for (c) used \texttt{-bvp\_c 0}; for (f) used \texttt{-pc\_factor\_mat\_solver\_type mumps}):
...
\paragraph{(c)} CG with $c=0$: the matrix $A$ is unchanged; iteration count often matches (b). In our run (c) converged in 1 iteration (atol); on another run it may match (b).

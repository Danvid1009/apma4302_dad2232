...
HPC reductions, calibrated volatility, and PETSc PDE / Krylov}
...
\begin{abstract}
This report documents a deliberately broad scientific computing project that
ties together three PETSc-aware threads: \textbf{(i)} scalable Monte Carlo estimation with
...
Black--Scholes as a clean nonlinear equation example. 
\medskip

The second phase connects the same Monte Carlo engine to \textbf{historical SPY}
(SPDR S\&P 500 ETF) daily prices from Yahoo Finance~\cite{yahoofinance,yfinance,spy-etf}:
rolling realized volatility replaces a toy constant $\sigma$, and a roll-forward
experiment reprices an ATM-style call along a sparse calendar of market snapshots.

\medskip
The goal of this project was neither finance nor PDE theory for their own
sake: using both as for practicing a single style of work at
...
\medskip
\noindent\textbf{Citations overview.}
Options theory follows Black--Scholes and Merton~\cite{blackscholes,merton1973};
Phase~2 market inputs use SPY adjusted closes from Yahoo Finance via
\texttt{yfinance}~\cite{yahoofinance,yfinance,spy-etf};
...
\section{Introduction: problem and claims}

\subsection{The motivating tension}
Monte Carlo (MC) option pricing is attractive because it scales well in the
...
but they share a common engineering spine: partition data, compute locally,
...
We claim three research outcomes supported by artifacts in the report
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Historical realism:} realized volatility from historical SPY data is
...
\subsection{Where the ideas came from}
The financial modeling layer is textbook GBM risk-neutral pricing as
presented in foundational derivatives references~\cite{blackscholes,merton1973}.
The PETSc object model (Vec, Mat, KSP, PC, SNES) and the habit of treating
performance as a first-class output follow the PETSc team's user manual and the
original PETSc software paper, and class (of course) ~\cite{petsc-user-ref,petsc-efficient}. The
PDE discretization + Krylov + Newton narrative is intentionally aligned with
...
The visualization layer follows standard ParaView practice for scientific
vector/tensor data~\cite{paraview-guide}.
...
\subsection{Grid PDEs as the solver-dominated workload}
The PDE thread supplies the \textbf{sparse linear algebra} side of the project:
each model is a standard classroom example from Bueler~\cite{bueler2020petsc}, chosen
so the same PETSc objects (\textsf{Mat}, \textsf{Vec}, \textsf{KSP}, \textsf{PC})
appear in settings that differ in symmetry and time structure:
\begin{align*}
  -\Delta u &= f && \text{(Poisson: one elliptic solve, spatial convergence)},\\
  u_t - \kappa \Delta u &= g && \text{(heat: repeated SPD steps, ParaView movies)},\\
  u_t + \mathbf{v}\!\cdot\!\nabla u - \nu \Delta u &= s &&
  \text{(advection--diffusion: nonsymmetric shifts, parallel-safe PC choices)}.
\end{align*}
Structured five-point stencils on the unit square discretize all three; implicit
Euler in time reduces the parabolic problems to a linear system per step. The
point for this report is not PDE theory but repeatable solver practice:
assemble, solve with instrumented \textsf{KSP}, export \texttt{.vtp}/\texttt{.pvd}
for inspection, and record iteration counts alongside wall time~\cite{bueler2020petsc,petsc-user-ref}.
...
\section{Ground Truth: why historical volatility is scientifically meaningful}
A constant $\sigma$ is a modeling convenience, not a claim about markets. Phase~2
replaces that convenience with rolling realized volatility estimated from
log returns over a trailing window of trading days (data provenance in
Section~\ref{sec:data-sources}), then feeds the resulting
$\sigma_{\mathrm{roll}}(t)$ into the same MC pricer used in Phase~1.

This connects the project to data conditioning and nonstationarity:
...
are intentionally modest comparative experiments: they record iteration
counts and wall times under controlled changes to solver configuration. The
research conclusion is not ``Jacobi always wins''---it is that solver
...
This section embeds representative ParaView (or ParaView-equivalent) stills
...
The trajectory stills connect the Monte Carlo story to geometry: how
...
\begin{figure}[!h]
  ...
  experiments into one glanceable page. It summarizes timings and
  speedups from the repository's scaling drivers. Parallel
  MC is not automatically linear speedup---collectives, load balance, and fixed problem
  size all bend the curves. Near-linear segments mean
...
\newpage
\section{Figures: Historical inputs and roll-forward}
...
\begin{figure}[!h]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{phase2_spy_price_and_vol.png}
  ...
  \begin{subfigure}[t]{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{phase2_roll_forward_panel.png}
  ...
  \caption[Phase~2 input and roll-forward summary]{Phase~2 market input and roll-forward repricing summary.}
...
\begin{figure}[!h]
  ...
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{strong_scaling_speedup_errorbars.png}
    \caption[Strong scaling speedup]{Strong scaling speedup with error bars.}
    \label{fig:strong-speedup}
  \end{minipage}
  \hfill
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{strong_scaling_runtime_errorbars.png}
    \caption[Strong scaling runtime]{Strong scaling runtime variability by rank count.}
    \label{fig:strong-runtime}
  \end{minipage}
  \hfill
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{strong_scaling_efficiency.png}
    \caption[Strong scaling efficiency]{Strong scaling efficiency vs.\ ranks.}
...
\subsection{Historical data conclusions}
...
Table~\ref{tab:data-sources} lists every \emph{external} input used in this report.
Monte Carlo paths, Poisson/heat/advection grids, scaling logs, and ParaView geometry
are \textbf{generated in-repo} from models and scripts (not third-party downloads).

\begin{table}[ht]
  \centering
  \footnotesize
  \begin{tabular}{@{}p{0.22\linewidth}p{0.28\linewidth}p{0.18\linewidth}p{0.26\linewidth}@{}}
    \toprule
    \textbf{Use in report} & \textbf{Provider / method} & \textbf{File} & \textbf{Notes} \\
    \midrule
  Phase~2 SPY price \& rolling $\sigma$ (Fig.~\ref{fig:phase2-spy}) &
  Yahoo Finance daily OHLCV via \texttt{yfinance}~\cite{yfinance,yahoofinance} &
  \texttt{data/raw/SPY.csv} &
  SPY (SPDR S\&P 500 ETF)~\cite{spy-etf}; Adj Close for returns; 252-day rolling vol \\
  Phase~2 calibration \& roll-forward &
  same CSV + in-repo MC engine &
  \texttt{SPY.csv}; JSON under \texttt{output/} &
  $r{=}4.5\%$ stated modeling assumption; strikes near ATM on snapshot dates \\
  Offline Phase~2 demo &
  hand-built synthetic series &
  \texttt{example\_synthetic\_ohlcv.csv} &
  not market data; for CI/tests without network \\
  Optional quote check &
  user-filled template &
  \texttt{quotes\_template.csv} &
  illustrative mids only; not used in figures unless populated \\
    \bottomrule
  \end{tabular}
  \caption{External and template data inputs. All other artifacts (MC paths, PDE fields, timing CSVs) are computed locally.}
  \label{tab:data-sources}
\end{table}

\subsection{Historical SPY OHLCV (Phase~2)}
The equity series is the \textbf{SPDR S\&P 500 ETF (ticker SPY)}---a listed product
whose adjusted prices are intended to reflect distributions and splits~\cite{spy-etf}.
We downloaded daily open/high/low/close/volume and \textbf{adjusted close} for
2018-01-02 through 2024-12-30 (1{,}760 trading rows) using
\texttt{scripts/fetch\_yahoo\_ohlcv.py}, which calls the unofficial
\texttt{yfinance} client~\cite{yfinance} against Yahoo Finance's public quote
history~\cite{yahoofinance}. The committed snapshot \texttt{data/raw/SPY.csv} is the
canonical copy for figures and calibration; re-fetch only if you need to refresh the
end date.

\textbf{Columns used.} Log returns and annualized realized volatility use
\textbf{Adj Close} (split/dividend adjusted). Spot $S_0$ for repricing experiments
defaults to the last available Adj Close in the file unless overridden with
\texttt{--s0}. Rolling volatility in
\texttt{plot\_spy\_volatility\_regime.py} applies a trailing window of
\textbf{252 trading days} and scales by $\sqrt{252}$.

\textbf{Reported option scenario} (Tier~A runs in \texttt{RESULTS\_PHASE1.md}):
European call, $K{=}580$ (rounded ATM vs.\ last spot $\approx 579.81$ on 2024-12-30),
$T{=}0.25$ years, $r{=}0.045$, $N{=}200{,}000$ paths, baseline $\sigma{=}0.20$ vs.\
$\sigma_{\mathrm{cal}}$ from either the full history ($\approx 0.195$) or the last
252 rows ($\approx 0.126$).

\subsection{Synthetic and optional market templates}
\texttt{data/raw/example\_synthetic\_ohlcv.csv} is a small fabricated OHLCV file
(shipped for offline runs of \texttt{phase2\_calibrate\_and\_price.py} without
network access). \texttt{data/raw/quotes\_template.csv} is a two-row scaffold for
\texttt{compare\_mc\_to\_quotes.py}; populate it with your own quote snapshot if you
extend the quote-comparison item in Section~\ref{sec:remaining}.

\subsection{Computed data (no external provider)}
Phase~1 GBM paths, barrier sweeps, Poisson/heat/advection fields, KSP/PC sweep logs,
and strong/weak scaling tables are produced by repository scripts documented in
\texttt{PROJECT\_OUTLINE.md} and \texttt{docs/DATA\_SOURCES.md}. ParaView
\texttt{.vtp}/\texttt{.pvd} bundles are regenerated locally (large \texttt{.vtp}
files are git-ignored; see \texttt{output/paraview/vtp/README.md}).

\paragraph{Compliance note.}
\texttt{yfinance} is convenient for coursework but is not an exchange or vendor
feed; cite Yahoo as the upstream portal~\cite{yahoofinance}, cache CSVs under
\texttt{data/raw/}, and respect Yahoo's terms of use and rate limits when re-downloading.
...
\bibitem{yfinance}
R. Aroussi.
\newblock \texttt{yfinance}: Yahoo!\ finance market data downloader (Python package).
\newblock \url{https://github.com/ranaroussi/yfinance}, 2017--present.
\newblock Accessed May 2025.

\bibitem{yahoofinance}
Yahoo.
\newblock Yahoo Finance --- historical quotes and company data.
\newblock \url{https://finance.yahoo.com/}.
\newblock Accessed May 2025 for SPY daily OHLCV retrieval.

\bibitem{spy-etf}
State Street Global Advisors.
\newblock SPDR S\&P 500 ETF Trust (SPY) --- fund overview and fact sheet.
\newblock \url{https://www.ssga.com/us/en/institutional/etfs/funds/spdr-sp-500-etf-trust-spy}.
\newblock Accessed May 2025.
